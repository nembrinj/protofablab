...
public:
  ...
  void ApplicationFunctionSet_Tracking(int);           //Line Tracking Mode
  ...
  void ApplicationFunctionSet_SerialPortDataAnalysis(void);  //Data processing from elegoo interface that isn't used
  void ApplicationFunctionSet_IRrecv(void);
  void TeddyCtrlUARTMessage(String);    // handle message received from ESP via UART
  void TeddyCtrlManager(int, int);        // handle instructions for motors and servo based on UART messages
...
private:
  ...
  /*UART message and instructions for motors and servo*/
  int Aiming(int, int);
  int TurnServo(int);
  void RecalibrateServo(unsigned long, unsigned long, int, int);
  void MotorHandling(int, int, int, int);
